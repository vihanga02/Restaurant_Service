...
const Desserts = () => {
    ...
    const filteredDesserts = desserts.filter(dessert => {
        const matchesSearch = dessert.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            (dessert.description && dessert.description.toLowerCase().includes(searchTerm.toLowerCase()));
        const matchesFavorite = !showFavoritesOnly || favoriteIds.includes(dessert._id);
        const matchesRating = (dessert.starRating || 0) >= minRating;
