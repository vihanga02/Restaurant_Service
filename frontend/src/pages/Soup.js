...
const Soup = () => {
    ...
    const filteredSoups = soups.filter(soup => {
        const matchesSearch = soup.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            (soup.description && soup.description.toLowerCase().includes(searchTerm.toLowerCase()));
        const matchesFavorite = !showFavoritesOnly || favoriteIds.includes(soup._id);
        const matchesRating = (soup.starRating || 0) >= minRating;
